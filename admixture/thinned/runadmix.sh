...
for species in "Tory" "Tder" "Tstr" "Timb" "Tspe" ## removed Tbux, only one location
#for species in "Scaf"
do
...
plink --allow-extra-chr --bfile $inputpath --keep keep_list.txt --maf 0.01 --geno 0.1 --make-bed --out plink --thin 0.1
...
CONV=`Rscript -e "r<-read.table('./ll.$i.txt');r<-r[order(-r[,1]),];cat(sum(r[1]-r<3),'\n')"` # how many top ll's are roughly equal
