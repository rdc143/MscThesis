...
# plot R2 >0.9 vs MAF
dat = as.data.frame(matrix(unlist(parallel::mclapply(1:50, function(x){d = subset(df, bin==x & R2>0.9); m = mean(as.numeric(d$R2)); n = length(d$R2); o = c(x, m, n)}, mc.cores=20)), ncol=3, byrow=T))
plotInfoMaf(dat, "R2", paste("R2(>0.9) vs MAF \n#Sites: ", sum(dat$V3)))
